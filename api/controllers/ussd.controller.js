...
// ---- helpers (kept, but not used in this minimal test) ----
const getCustomerDetails = async (client) => {
  const customerDetails = {
    ...(await getSpecificCustomer(client)),
    ...(await getTISPCustomer(client)),
...
// ---- minimal USSD handler (ONLY welcome + exit) ----
const initiateUSSD = async (req, res) => {
  try {
    const { phoneNumber, text = "" } = req.body || {};

    // helpful debug
    console.log("USSD HIT:", { phoneNumber, text });

    const input = String(text).trim();

    // Keep response format EXACT: CON continues, END terminates
    const welcome = `CON Welcome to Starlynx Utility Limited.
1. Say Hi
0. Exit`;

    // first screen
    if (input === "" || input === "99") {
      return send(res, welcome);
    }
    ...
    // option 1
    if (input === "1") {
      return end(res, "Hi! USSD endpoint is working.");
    }

    // anything else
    return end(res, "Invalid choice.");
  } catch (err) {
    console.error("USSD ERROR:", err);
    return end(res, "System error. Please try again.");
  }
};
