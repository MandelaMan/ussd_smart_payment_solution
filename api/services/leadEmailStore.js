...
async function createMessage({
  ...
  const result = await query(
    `INSERT INTO lead_email_messages
      (lead_id, direction, from_address, to_address, subject, body_html, body_text,
  ...
  const rows = await query(`SELECT * FROM lead_email_messages WHERE id = ?`, [id]);
  return formatRow(rows[0]);
}

async function listByLeadId(leadId, { limit = 80 } = {}) {
  const rows = await query(
    `SELECT * FROM lead_email_messages
     WHERE lead_id = ?
     ORDER BY created_at ASC, id ASC
     LIMIT ?`,
    [leadId, Math.min(200, Math.max(1, limit))]
  );
